...
void main() {
  ...
  group('UserRepository Tests', () {
    test('Should login successfully with valid credentials', () async {
      ...
      debugPrint('Access Token: ${response['access_token'].substring(0, 20)}...');
    ...
    test('Should fail login with invalid credentials', () async {
      try {
        ...
        debugPrint('[PASS] Invalid login correctly rejected');
      }
    });
    ...
    test('Should check if user is logged in after login', () async {
      ...
      debugPrint('[PASS] Login state persisted');
    });
    ...
    test('Should get current user after login', () async {
      ...
      debugPrint('[PASS] Current user retrieved: ${user.username}');
    });
    ...
    test('Should logout and clear tokens', () async {
      ...
      debugPrint('[PASS] Logout successful, tokens cleared');
    });
  });
  ...
  group('LeagueRepository Tests (requires auth)', () {
    ...
    test('Should fetch all leagues', () async {
      ...
      debugPrint('[PASS] Fetched ${leagues.length} leagues');
    ...
    test('Should fetch specific league by ID', () async {
      ...
      debugPrint('[PASS] Fetched league: ${league.name}');
    });
    ...
    test('Should fetch seasons for a league', () async {
      ...
      debugPrint('[PASS] Fetched ${seasons.length} seasons for ${leagues.first.name}');
    ...
    test('Should fetch standings for a season', () async {
      ...
      debugPrint('[PASS] Fetched ${standings.length} standings for ${seasons.first.name}');
  ...
  group('MatchRepository Tests (requires auth)', () {
    setUp(() async {
      await userRepo.login('league_admin', 'AdminPass123');
    });
    ...
    test('Should fetch all matches', () async {
      ...
      debugPrint('[PASS] Fetched ${matches.length} matches');
    ...
    test('Should fetch matches filtered by season', () async {
      ...
      debugPrint('[PASS] Fetched ${matches.length} matches for season ${seasons.first.name}');
    });
    ...
    test('Should fetch specific match by ID', () async {
      ...
      debugPrint('[PASS] Fetched match ${match.matchId}: ${match.status}');
    });
    ...
    test('Should fetch sets for a match', () async {
      ...
      debugPrint('[PASS] Fetched ${sets.length} sets for match ${matches.first.matchId}');
      for (var set in sets) {
  ...
  group('Authorization Tests', () {
    test('Should get 401 when accessing protected endpoint without login', () async {
      ...
      try {
        ...
        expect(e.toString(), contains('Unauthorized'));
        debugPrint('[PASS] Unauthorized access correctly blocked');
      }
    });
    ...
    test('Should automatically refresh token when expired', () async {
      ...
      debugPrint('[PASS] Token expiry check working: expired = $isExpired');
  ...
  group('Session Persistence Tests', () {
    test('Should restore session from stored tokens', () async {
      ...
      debugPrint('[PASS] Session restored successfully');
      
      final newLeagueRepo = LeagueRepository(newApiClient);
      final leagues = await newLeagueRepo.getLeagues();
      expect(leagues, isNotEmpty);
      debugPrint('[PASS] Authenticated request successful after session restore');
